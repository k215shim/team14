class CollisionHandler {
  ...
  void checkY(Hitbox hb, Character c) {
    ...
    if (dy > 0) { // 落下
      ...
      for (int tx = minTileX; tx <= maxTileX; tx++) {
        if (checker.isTileSolidMap(tx, ty)) {
          hb.y = (ty + 1) * checker.tileSize - 10;
          hb.stopY();
          return;
        }
        if (checker.isTileSolidObj(tx, ty)) {
          hb.y = (ty + 1) * checker.objTileSize - 8;
          hb.stopY();
          return;
        }
      }
  ...
  boolean willCollideX(Hitbox hb) {
    float nextX = hb.x + hb.vx;
    float top = hb.y;
    float bottom = hb.y + hb.h;

    int maxTileY = int(top / checker.tileSize);
    int minTileY = int((bottom - 3) / checker.tileSize);
    ...
    if (hb.vx > 0) { // 右
      ...
      int tx = int(nextX / checker.tileSize);
      for (int ty = minTileY; ty <= maxTileY; ty++) {
        if (checker.isTileSolidMap(tx, ty) || checker.isTileSolidObj(tx, ty)) {
          return true;
        }
      }
    }
  ...
  boolean willCollideY(Hitbox hb) {
    float nextY = hb.y + hb.vy;
    float left = hb.x;
    float right = hb.x + hb.w;

    int maxTileX = int(right / checker.tileSize);
    int minTileX = int((left - 3) / checker.tileSize);
