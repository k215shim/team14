...
int chargeLevel = 0;
  ...
  void stopChargingAndAttack() {
    ...
    if (chargeLevel == 0) {
      ...
      fireballX, fireballY, faceLeft ? -6 : 6, -4, fireballFrames));
    }
      ...
      if (faceLeft) fireArrows.add(new FireArrowEffect(spawnX, spawnY, speed, fireArrowLeftFrames));
      else fireArrows.add(new FireArrowEffect(spawnX, spawnY, speed, fireArrowFrames));
    }
    else {
      attackDuration = 40;
      ...
      if (faceLeft){
        infernos.add(new InfernoEffect(fireX, fireY, infernoVX, infernoLeftFrames));
        inpacts.add(new Inpact(fireX, fireY - 10, inpactVX, inpactLeftFrames));
      }
      else {
        infernos.add(new InfernoEffect(fireX, fireY, infernoVX, infernoFrames));
