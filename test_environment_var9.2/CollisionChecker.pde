class CollisionChecker {
  ...
  boolean isWallCollision(float x, float y, int w, int h) {
    int leftTileX = int(x / tileSize);
    int rightTileX = int((x + w - 1) / tileSize);
    int topTileY = int(y / tileSize);
    int bottomTileY = int((y + h - 1) / tileSize);

    // 左壁チェック
    for (int ty = topTileY; ty <= bottomTileY; ty++) {
      if (isTileSolidMap(leftTileX, ty) || isTileSolidObj(leftTileX, ty)) {
        return true;
      }
    }
    ...
    // 右壁チェック
    for (int ty = topTileY; ty <= bottomTileY; ty++) {
      if (isTileSolidMap(rightTileX, ty) || isTileSolidObj(rightTileX, ty)) {
        return true;
      }
    }
